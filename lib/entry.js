  ...
  this.regurgitate = function(cb) {
    next_id('entry', this._id, function(id) {
      arrange_topics(id, function(that) {
        var doc = {_id: id, created_at: created_at(), entry: that.entry, subject: that.subject, topic_ids: that.topic, user_id: 1};
        db.entry.update({_id: doc._id}, doc, {upsert: true}, function() {
          cb();
        });
      });
    });
